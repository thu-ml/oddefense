...
class BaseDetector(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self, init_cfg=None, adv_cfg=None):
        ...
        if self.adv_cfg is not None:
            ...
            self.epsilon = self.adv_cfg.get("epsilon", 8)
    ...
    def train_step(self, data, optimizer):
        ...
        if self.adv_flag:
            ...
            if self.adv_noise is None:
                ...
                self.adv_noise = torch.zeros(data['img'].shape, device=device)
            ...
            self.aux_img = img_ori + self.adv_noise
            ...
            # if self.cnt < 40:
            #     if self.cnt % 4 == 1:
            #         cv2.imwrite("raw%s.png" % (int(self.cnt)), (gg[1].permute(1,2,0).squeeze(0).detach().cpu().numpy()[:,:,(2,1,0)]))
            # else:
            #     exit(0)
            ...
            # cv2.imwrite(str(self.cnt) + 'raw.png', (self.aux_img[2].permute(1,2,0).squeeze(0).detach().cpu().numpy()[:,:,(2,1,0)]))
