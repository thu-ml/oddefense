...
##############################################################################
        # prepare for dn
        if 'adv_flag' in img_metas[0] and img_metas[0]['adv_flag']:
            input_query_label, input_query_bbox, attn_mask, mask_dict = \
                prepare_for_dn(dn_args, embedweight, x.size(0), True, self.num_query, self.num_classes,
...
##################################################################################################
    ...
    def loss(self,
        ...
        for loss_cls_i, loss_bbox_i, loss_iou_i in zip(losses_cls[:-1],
                                                       losses_bbox[:-1],
                                                       losses_iou[:-1]):
            if len(loss_cls_i.shape) > 0:
                loss_dict[f'd{num_dec_layer}.loss_cls'] = torch.mean(loss_cls_i)
                loss_dict[f'd{num_dec_layer}.loss_bbox'] = torch.mean(loss_bbox_i)
            else:
                loss_dict[f'd{num_dec_layer}.loss_cls'] = loss_cls_i
                loss_dict[f'd{num_dec_layer}.loss_bbox'] = loss_bbox_i
            loss_dict[f'd{num_dec_layer}.loss_iou'] = loss_iou_i
    ...
    def loss_single(self,
        ...
        if "cwa" in img_metas[0] and img_metas[0]["cwa"]:
            ...
            loss_cls = self.loss_cls(
                cls_scores, labels, label_weights, avg_factor=cls_avg_factor, reduction_override='none')
            loss_cls = loss_cls * loss_cls.new_tensor(classcnt)
        else:
            loss_cls = self.loss_cls(
                cls_scores, labels, label_weights, avg_factor=cls_avg_factor, reduction_override='none')
        ...
        if self.clip is not None:
            high_inds = loss_cls > self.clip
            loss_cls[high_inds] = self.clip * loss_cls[high_inds] / loss_cls[high_inds].detach()
        
        if "mtd" not in img_metas[0]:
            loss_cls = torch.sum(loss_cls) / cls_avg_factor
        else:
            loss_cls = loss_cls.shape[0] * torch.sum(loss_cls, dim = 1) / cls_avg_factor
        ...
        if "mtd" not in img_metas[0]:
            ...
            loss_bbox = self.loss_bbox(
                ...
                avg_factor=num_total_pos, reduction_override='none')

            loss_bbox = loss_bbox.sum(dim=1) * loss_bbox.shape[0] / num_total_pos
