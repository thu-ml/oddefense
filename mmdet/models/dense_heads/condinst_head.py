...
def tensor2imgs(tensor, mean=None, std=None, to_rgb=True):
    ...
    for img_id in range(num_imgs):
        img = tensor[img_id, ...].detach().cpu().numpy().transpose(1, 2, 0).astype(np.float32)
...
class CondInstBoxHead(AnchorFreeHead):
    ...
    def loss(self,
        ...
        if "cwa" in img_metas[0] and img_metas[0]["cwa"]:
            loss_cls = self.loss_cls(
                flatten_cls_scores, flatten_labels, avg_factor=num_pos, reduction_override='none')
        ...
        if "mtd" not in img_metas[0]:
            loss_cls = torch.sum(loss_cls) / num_pos
        else:
            loss_cls = loss_cls.shape[0] * torch.sum(loss_cls, dim = 1) / num_pos
        ...
        if len(pos_inds) > 0:
            ...
            if "mtd" not in img_metas[0]:
                ...
                loss_bbox_base = torch.zeros_like(loss_cls, device = loss_cls.device)
                loss_bbox = self.loss_bbox(
                    pos_decoded_bbox_preds,
                    pos_decoded_target_preds,
                    weight=pos_centerness_targets,
                ...
                loss_bbox_base[pos_inds] += loss_bbox / centerness_denorm
                loss_bbox = loss_bbox_base * loss_bbox_base.shape[0]
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        if num_gts == 0:
            # print(gt_labels.new_full((num_points,), self.num_classes).shape)
            # print(gt_labels.new_full((num_points,), -1))
            return gt_labels.new_full((num_points,), self.num_classes), \
                   gt_bboxes.new_zeros((num_points, 4)), \
                   gt_labels.new_full((num_points,), 0)
