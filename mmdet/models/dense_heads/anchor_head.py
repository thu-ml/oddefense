...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            ...
            # set different weight for different category
            if "cwa" in img_meta and img_meta["cwa"]:
                classcnt = img_meta["classcnt"]
                weightsum  = np.sum(classcnt)

                for i in range(len(classcnt)):
                    category_pos_inds = pos_inds[sampling_result.pos_gt_labels==i]
                    label_weights[category_pos_inds] = weightsum / classcnt[i]
                    bbox_weights[category_pos_inds, :] = weightsum / classcnt[i]
                label_weights[pos_inds] = len(label_weights[pos_inds]) * label_weights[pos_inds] / torch.sum(label_weights[pos_inds])
                bbox_weights[pos_inds] = len(bbox_weights[pos_inds]) * bbox_weights[pos_inds] / torch.sum(bbox_weights[pos_inds])
