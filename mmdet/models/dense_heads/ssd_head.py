...
class SSDHead(AnchorHead):
    ...
    def loss_single(self, cls_score, bbox_pred, anchor, labels, label_weights,
        ...
        if targeted:
            ...
            target_labels[labels == 14] = 16
            target_labels[labels == 13] = 1

            loss_cls_all_target = -F.cross_entropy(
                cls_score, target_labels, reduction='none') * label_weights
            loss_cls_all = F.cross_entropy(
                cls_score, labels, reduction='none') * label_weights
            loss_cls_all[labels == 14] = loss_cls_all_target[labels == 14] + loss_cls_all[labels == 14]
            loss_cls_all[labels == 13] = loss_cls_all_target[labels == 13] + loss_cls_all[labels == 13]
            loss_cls_all_target = loss_cls_all
        else:
            loss_cls_all = F.cross_entropy(
                cls_score, labels, reduction='none') * label_weights
        
        if self.clip is not None:
            high_inds = loss_cls_all > self.clip
            loss_cls_all[high_inds] = self.clip * loss_cls_all[high_inds] / loss_cls_all[high_inds].detach()
        ...
        if targeted:
            loss_cls_pos = loss_cls_all_target[pos_inds].sum()
        else:
