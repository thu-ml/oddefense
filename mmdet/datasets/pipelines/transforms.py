...
class CopyPasteObj:
    ...
    def __init__(
        ...
        self.bbox_occluded_thr = bbox_occluded_thr
        self.selected = selected
        self.perturb_pixel = perturb_pixel
    ...
    def _select_object(self, results):
        ...
        bboxes = results['gt_bboxes']
        labels = results['gt_labels']
        max_num_pasted = min(bboxes.shape[0] + 1, 100)
        num_pasted = np.random.randint(0, max_num_pasted)
        selected_inds = np.random.choice(
            bboxes.shape[0], size=num_pasted, replace=False)
    ...
    def _copy_paste(self, dst_results, src_results):
        ...
        patchlist = []
        hweps = []
        prs = [random.randint(self.perturb_pixel) for i in range(4)]
        for box in src_bboxes:
            y1, y2 = max(int(box[1])-prs[0], 0), min(int(box[3])+prs[1], src_img.shape[1])
            x1, x2 = max(int(box[0])-prs[2], 0), min(int(box[2])+prs[3], src_img.shape[0])
            patchlist.append(src_img[y1:y2, x1:x2])
            hweps.append((box[0]-x1, box[1]-y1))

        rh, rw, _ = dst_img.shape
        for i in range(len(src_bboxes)):
            ...
            while(cnt > 0):
                ...
                if not inter:
                    ...
                    dst_results['img'][int(box[1]):int(box[1])+sh, int(box[0]):int(box[0])+sw] = patchlist[i]
                    box = np.array([x+hweps[i][0], y+hweps[i][1], x + w + hweps[i][0], y + h + hweps[i][1]]).astype(np.float32)
                    dst_results['gt_bboxes'] = np.vstack((dst_results['gt_bboxes'], box))
                    dst_results['gt_labels'] = np.hstack((dst_results['gt_labels'], src_labels[i]))
                    break
                else:
                    cnt -= 1
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'max_time={self.max_time}, '
        repr_str += f'bbox_occluded_thr={self.bbox_occluded_thr}, '
        repr_str += f'selected={self.selected}, '
        return repr_str
